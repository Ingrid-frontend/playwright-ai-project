/**
 * @spec-meta
 * {"playwrightEnv":"stage","accountProfile":"default","loginAccount":null,"recordedAt":"2026-08-13T06:31:48.052Z"}
 */
...
test('test', async ({ page }) => {
  ...
  const screenshotDir = withScreenshotRunSegment('screenshots/stage/260814/账本_2026-08-13_14-31-48');
  ...
  await step('账本', async () => {
    ...
    const locator = baseContext.getByText('账本').filter({ visible: true }).first();
    try {
      await expect(locator).toBeVisible({ timeout: 4000 });
    } catch {
      console.log('ℹ️  账本：元素未出现（非关键步骤），跳过本步');
      await takeStepScreenshot(page, path.join(runDir, `step-2-账本-skipped.png`), { mode: 'stable' });
      return;
    }
  ...
  await step('action', async () => {
    ...
    await page.locator('iframe').first().waitFor({ state: 'attached', timeout: 12000 }).catch(() => {});
    const baseContext = page.frameLocator('iframe').first();
    const locator = baseContext.locator('.warp-svg-icon > .helios-icon').first();
    try {
      await expect(locator).toBeVisible({ timeout: 4000 });
    } catch {
