...
test('test', async ({ page }) => {
  ...
  const screenshotDir = withScreenshotRunSegment('screenshots/20260515/预提单_2026-04-24_15-11-19');
  ...
  await step('预提单', async () => {
    await takeStepScreenshot(page, path.join(runDir, `step-2-预提单-before.png`));
    const locator = page.getByText('预提单').filter({ visible: true }).first();
    await expect(locator).toBeVisible({ timeout: 5000 });
    await smartClick(locator, '预提单');
    await page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
    await waitForPostInteractionPaint(page);
    await takeStepScreenshot(page, path.join(runDir, `step-2-预提单-after.png`), { mode: 'stable' });
  });
  ...
  await step('导入费用', async () => {
    await takeStepScreenshot(page, path.join(runDir, `step-5-导入费用-before.png`));
    await page.locator('iframe').last().waitFor({ state: 'attached', timeout: 20000 }).catch(() => {});
    const _childFrames = page.frames().filter((f) => f !== page.mainFrame());
    const baseContext = _childFrames.length > 0 ? _childFrames[_childFrames.length - 1]! : page.mainFrame();
    const locator = baseContext.getByRole('button', { name: '导入费用' }).filter({ visible: true }).first();
    await expect(locator).toBeVisible({ timeout: 25000 });
    await smartClick(locator, '导入费用');
    await page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
    await waitForPostInteractionPaint(page);
    await takeStepScreenshot(page, path.join(runDir, `step-5-导入费用-after.png`), { mode: 'stable' });
  });
