...
test('test', async ({ page }) => {
  ...
  const screenshotDir = withScreenshotRunSegment('screenshots/20260515/预算单_2026-04-24_15-19-13');
  ...
  await step('预算单', async () => {
    await takeStepScreenshot(page, path.join(runDir, `step-2-预算单-before.png`));
    const locator = page.getByText('预算单').filter({ visible: true }).first();
    await expect(locator).toBeVisible({ timeout: 5000 });
    await smartClick(locator, '预算单');
    await page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
    await waitForPostInteractionPaint(page);
    await takeStepScreenshot(page, path.join(runDir, `step-2-预算单-after.png`), { mode: 'stable' });
  });
  ...
  await step('预算占用调整单', async () => {
    await takeStepScreenshot(page, path.join(runDir, `step-4-预算占用调整单-before.png`));
    await page.locator('iframe').last().waitFor({ state: 'attached', timeout: 20000 }).catch(() => {});
    const _childFrames = page.frames().filter((f) => f !== page.mainFrame());
    const baseContext = _childFrames.length > 0 ? _childFrames[_childFrames.length - 1]! : page.mainFrame();
    const locator = baseContext.getByRole('tab', { name: '预算占用调整单' }).filter({ visible: true }).first();
    await expect(locator).toBeVisible({ timeout: 25000 });
    await smartClick(locator, '预算占用调整单');
    await page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
    await waitForPostInteractionPaint(page);
    await takeStepScreenshot(page, path.join(runDir, `step-4-预算占用调整单-after.png`), { mode: 'stable' });
  });

  await step('预算编制单', async () => {
    await takeStepScreenshot(page, path.join(runDir, `step-5-预算编制单-before.png`));
    await page.locator('iframe').last().waitFor({ state: 'attached', timeout: 20000 }).catch(() => {});
    const _childFrames = page.frames().filter((f) => f !== page.mainFrame());
    const baseContext = _childFrames.length > 0 ? _childFrames[_childFrames.length - 1]! : page.mainFrame();
    const locator = baseContext.getByRole('tab', { name: '预算编制单' }).filter({ visible: true }).first();
    await expect(locator).toBeVisible({ timeout: 25000 });
    await smartClick(locator, '预算编制单');
    await page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
    await waitForPostInteractionPaint(page);
    await takeStepScreenshot(page, path.join(runDir, `step-5-预算编制单-after.png`), { mode: 'stable' });
  });
