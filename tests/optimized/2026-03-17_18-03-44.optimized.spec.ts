...
test("test", async ({ page }) => {
  ...
  await test.step("step-1-请选择日期", async () => {
    const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(1, "before-请选择日期"),
    );
    console.log("📍 当前路由:", beforeRoute);
    try {
      const _locator = page
        .locator("#home-main-charts-guide-id")
        .getByRole("textbox", { name: "请选择日期" })
        .filter({ visible: true });
    ...
    const { path: afterPath, route: afterRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(1, "after-请选择日期"),
    );
  ...
  await test.step("step-3-查看订单", async () => {
    const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(3, "before-查看订单"),
    );
    console.log("📍 当前路由:", beforeRoute);
    try {
      const _locator = page
        .locator("#helios-service-charts-guide-id-CHECK_INVOICE")
    ...
    const { path: afterPath, route: afterRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(3, "after-查看订单"),
    );
    console.log("📍 当前路由:", afterRoute);
  });
  await test.step("step-4-查看详情", async () => {
    const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(4, "before-查看详情"),
    );
    console.log("📍 当前路由:", beforeRoute);
    try {
      const _locator = page
    ...
    const { path: afterPath, route: afterRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(4, "after-查看详情"),
    );
    console.log("📍 当前路由:", afterRoute);
  });
  await test.step("step-5-action", async () => {
    const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(5, "before-action"),
    );
    console.log("📍 当前路由:", beforeRoute);
    try {
    ...
    const { path: afterPath, route: afterRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(5, "after-action"),
    );
    console.log("📍 当前路由:", afterRoute);
  });
  await test.step("step-6-action", async () => {
    const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(6, "before-action"),
    );
    console.log("📍 当前路由:", beforeRoute);
    try {
      const _locator = page.getByRole("img").filter({ visible: true }).first();
    ...
    const { path: afterPath, route: afterRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(6, "after-action"),
    );
    console.log("📍 当前路由:", afterRoute);
  });
  await test.step("step-7-按月份", async () => {
    const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(7, "before-按月份"),
    );
    console.log("📍 当前路由:", beforeRoute);
    try {
      const _locator = page
  ...
  await test.step("step-8-按季度", async () => {
    const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(8, "before-按季度"),
    );
    console.log("📍 当前路由:", beforeRoute);
    try {
      const _locator = page
        .getByRole("option", { name: "按季度" })
        .filter({ visible: true });
      await _locator.click({ force: true, delay: 100 });
    } catch (error) {
      console.log(
