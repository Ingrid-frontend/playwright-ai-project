...
test('test', async ({ page }) => {
  ...
  const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(page, getScreenshotPath(1, 'before-报销单'));
    console.log('📍 当前路由:', beforeRoute);
    try {
  const _locator = page.getByText("报销单").filter({ visible: true }).first();
  await _locator.click({ force: true, delay: 100 });
    } catch (error) {
      console.log(`❌ 步骤执行失败: ${error.message}`);
      throw error;
    }
  const { path: afterPath, route: afterRoute } = await screenshotWhenStable(page, getScreenshotPath(1, 'after-报销单'));
  ...
  const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(page, getScreenshotPath(2, 'before-action'));
    console.log('📍 当前路由:', beforeRoute);
    try {
  const _locator = page.locator(".anticon.anticon-close > svg").filter({ visible: true });
  await _locator.click({ force: true, delay: 100 });
    } catch (error) {
      console.log(`❌ 步骤执行失败: ${error.message}`);
      throw error;
  ...
  const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(page, getScreenshotPath(3, 'before-1'));
    console.log('📍 当前路由:', beforeRoute);
    try {
  await page.getByRole("cell", { name: "1", exact: true }).filter({ visible: true }).scrollIntoViewIfNeeded();
  ...
  const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(page, getScreenshotPath(4, 'before-办公用品类'));
    console.log('📍 当前路由:', beforeRoute);
    try {
  const _locator = page.getByText('办公用品类').filter({ visible: true }).first();
  await _locator.click({ force: true, delay: 100 });
    } catch (error) {
      console.log(`❌ 步骤执行失败: ${error.message}`);
      throw error;
    }
  const { path: afterPath, route: afterRoute } = await screenshotWhenStable(page, getScreenshotPath(4, 'after-办公用品类'));
    console.log('📍 当前路由:', afterRoute);
  });
  await test.step('step-5-取-消', async () => {
  const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(page, getScreenshotPath(5, 'before-取-消'));
    console.log('📍 当前路由:', beforeRoute);
    try {
  const _locator = page.getByRole("button", { name: "取 消" }).filter({ visible: true }).first();
  await _locator.click({ force: true, delay: 100 });
    } catch (error) {
      console.log(`❌ 步骤执行失败: ${error.message}`);
      throw error;
    }
  const { path: afterPath, route: afterRoute } = await screenshotWhenStable(page, getScreenshotPath(5, 'after-取-消'));
    console.log('📍 当前路由:', afterRoute);
  });
  await test.step('step-6-冲借款', async () => {
  const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(page, getScreenshotPath(6, 'before-冲借款'));
    console.log('📍 当前路由:', beforeRoute);
    try {
  const _locator = page.getByText('冲借款').nth(2).filter({ visible: true });
  await _locator.click({ force: true, delay: 100 });
    } catch (error) {
      console.log(`❌ 步骤执行失败: ${error.message}`);
      throw error;
    }
  const { path: afterPath, route: afterRoute } = await screenshotWhenStable(page, getScreenshotPath(6, 'after-冲借款'));
    console.log('📍 当前路由:', afterRoute);
  });
  await test.step('step-7-取-消', async () => {
  const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(page, getScreenshotPath(7, 'before-取-消'));
    console.log('📍 当前路由:', beforeRoute);
    try {
  const _locator = page.getByRole("button", { name: "取 消" }).filter({ visible: true }).first();
  await _locator.click({ force: true, delay: 100 });
  ...
  const { path: afterPath, route: afterRoute } = await screenshotWhenStable(page, getScreenshotPath(7, 'after-取-消'));
    console.log('📍 当前路由:', afterRoute);
  });
  await test.step('step-8-返-回', async () => {
  const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(page, getScreenshotPath(8, 'before-返-回'));
    console.log('📍 当前路由:', beforeRoute);
    try {
  const _locator = page.getByRole("button", { name: "返 回" }).filter({ visible: true }).first();
  await _locator.click({ force: true, delay: 100 });
    } catch (error) {
      console.log(`❌ 步骤执行失败: ${error.message}`);
      throw error;
    }
  const { path: afterPath, route: afterRoute } = await screenshotWhenStable(page, getScreenshotPath(8, 'after-返-回'));
    console.log('📍 当前路由:', afterRoute);
  });
