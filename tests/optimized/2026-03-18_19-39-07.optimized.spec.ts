...
test("test", async ({ page }) => {
  ...
  const screenshotRoot = "screenshots/2026-03-18_19-39-07";
  const now = new Date();
  ...
  await test.step("step-1-账本", async () => {
    const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(1, "before-账本"),
    );
    console.log("📍 当前路由:", beforeRoute);
    try {
      const _locator = page.getByText("账本").filter({ visible: true }).first();
  ...
  await test.step("step-2-1", async () => {
    ...
    const { path: afterPath, route: afterRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(2, "after-1"),
    );
    console.log("📍 当前路由:", afterRoute);
  });
  await test.step("step-3-取-消", async () => {
    const { path: beforePath, route: beforeRoute } = await screenshotWhenStable(
      page,
      getScreenshotPath(3, "before-取-消"),
    );
    console.log("📍 当前路由:", beforeRoute);
    try {
      const _locator = page
