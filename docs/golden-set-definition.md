...
| Intent | 文件 | 说明 | 试跑 |
|--------|------|------|------|
| 审批列表页可见 | `tests/definitions/stage/审批列表页可见.yaml` | 列表冒烟（对齐 Golden Set） | `intent:run --engine=pw` 全绿 |
| 审批列表页签可见 | `tests/definitions/stage/审批列表页签可见.yaml` | 待审批/我的已办/筛选 | 全绿 |
| 审批列表点击首行 | `tests/definitions/stage/审批列表点击首行.yaml` | 点击首行；后续步骤 `optional`（进详情交互未稳定） | 列表断言绿，点击后可选 |

```bash
npm run intent:run -- --intent=tests/definitions/stage/审批列表页可见.yaml --engine=pw --env=stage
npm run intent:run -- --intent=tests/definitions/stage/审批列表页签可见.yaml --engine=pw --env=stage
npm run intent:run -- --intent=tests/definitions/stage/审批列表点击首行.yaml --engine=pw --env=stage
```
