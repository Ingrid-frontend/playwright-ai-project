...
import { sendApiFailuresToFeishu } from './feishu-api-failure';
import { extractRequestFields, formatRequestFields } from './request-fields';
...
const DEFAULT_URL_IGNORES: RegExp[] = [
  ...
  /\/api\/company\/apportion\/template\/hit\/use\b/i,
];
...
export class ApiGuard {
  ...
  /** 丢弃 URL 匹配的失败（用于 available 等瞬时 500 重试成功后） */
  dismissMatching(urlRe: RegExp): number {
    let n = 0;
    for (let i = this.failures.length - 1; i >= 0; i -= 1) {
      if (urlRe.test(this.failures[i]!.url)) {
