...
const FLOW_ID = 'approval-flow' as const;
...
/**
 * 整次跑测共用一个浏览器窗口：用例之间不关窗，worker 结束才 close。
 * config.workers 必须为 1。
 */
export const test = base.extend<AppFixtures, WorkerFixtures>({
  _flowWorkerRun: [
    async ({}, use, workerInfo) => {
      const spec = process.env.FLOW_SPEC || 'approval/full-flow.spec.ts';
  ...
  approvalListPage: async ({ page }, use) => {
    await use(new ApprovalListPage(page));
  },
