...
function solidFill(node: FigmaNode): { hex: string; rgb: [number, number, number] } | null {
...
function relRect(node: FigmaNode, root: Rect | null): Rect {
...
function parseTextStyle(node: FigmaNode, colorHex?: string): TextStyle {
...
function isDynamicText(text: string, pathNames: string[]): boolean {
...
function classifyKind(pathNames: string[], dynamic: boolean): DesignText['kind'] {
...
function aggregateTexts(candidates: TextCandidate[]): DesignText[] {
...
function assignRegionKey(bbox: Rect, regions: DesignRegion[]): string | undefined {
...
function collectColors(node: FigmaNode, out: Map<string, { weight: number; shape: number; text: number }>): void {
...
function collectTypography(node: FigmaNode, root: Rect | null, out: Map<string, TypographyToken>): void {
...
function collectSpacing(node: FigmaNode, out: Map<string, SpacingToken>): void {
...
function collectRadii(node: FigmaNode, out: Map<number, number>): void {
...
function extractRegions(root: FigmaNode, rootRect: Rect, config: FigmaSpecConfig): DesignRegion[] {
