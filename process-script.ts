...
import { RawRecordingGenerator } from './scripts/recording/generate-raw-recording';
...
async function processScript() {
  ...
  await generator.generateFromCode(scriptContent, {
    name: '中控首页',
    description: '浏览'
  });
  
  // 清理临时文件
  if (fs.existsSync('temp-script.txt')) {
    fs.unlinkSync('temp-script.txt');
  }
