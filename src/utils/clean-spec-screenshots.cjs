...
const { parseOptimizedRel, buildScreenshotDir, getLegacyEnvDefault } = require("./test-env-path.cjs");
...
function resolveScreenshotDirRel(repoRoot, specRel) {
...
function assertSafeScreenshotDirRel(repoRoot, dirRel) {
...
function resolveDiffsDirRel(screenshotDirRel) {
...
function listRunTimestampEntries(screenshotDirAbs) {
  const entries = [];
...
function pickLatestRunEntries(entries, windowMs = LATEST_RUN_WINDOW_MS) {
 ...
 */
function cleanSpecScreenshots(repoRoot, specRel, opts = {}) {
...
module.exports = {
  RUN_SEGMENT_RE,
  LATEST_RUN_WINDOW_MS,
  resolveScreenshotDirRel,
  assertSafeScreenshotDirRel,
  resolveDiffsDirRel,
  listRunTimestampEntries,
  pickLatestRunEntries,
