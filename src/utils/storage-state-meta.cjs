 ...
 */
function readLoginStateMeta(storagePath) {
 ...
 * @returns {string|null}
 */
function formatLoginAccountLabel(meta) {
 ...
 * @returns {boolean}
 */
function annotateStorageStateMeta(storagePath, opts = {}) {
  ...
  const { extractFromCode, extractUserIdFromStorageData } = require("./extract-login-account.cjs");
