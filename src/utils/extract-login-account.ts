...
function isLikelyAccount(value?: any) {
...
function pickFromJson(obj?: any, preferId?: any) {
...
function scanString(value?: any) {
  if (!value || value.length > 800000) return null;

  const emails = value.match(EMAIL_RE);
  if (emails?.length) {
    const email = emails.find((e: string) => !/example\.com|test\.com/i.test(e));
...
function extractFromCode(code?: any) {
  if (!code) return null;
...
function extractUserIdFromStorageData(data?: any) {
  if (!data || typeof data !== "object") return null;
  for (const origin of data.origins || []) {
    const userIdItem = (origin.localStorage || []).find((x: { name?: string; value?: string }) => x.name === "hly.custom-browser-userId");
    if (userIdItem?.value) return String(userIdItem.value).trim();
  }
  return null;
}

function extractFromStorage(storagePath?: any) {
 ...
 * @returns {string|null}
 */
function extractLoginAccount(opts: any = {}) {
  return extractFromCode(opts.code) || extractFromStorage(opts.storagePath) || null;
}

export {
  extractLoginAccount, extractFromCode, extractFromStorage, extractUserIdFromStorageData,
};
