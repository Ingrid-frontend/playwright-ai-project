...
function isLoginActionLine(line?: any) {
  const s = line.trim();
...
function isInitialGotoLine(line?: any) {
  return /^\s*await\s+page\.goto\(/.test(line);
}

function isLoginTailLine(line?: any) {
  const s = line.trim();
  if (!s.startsWith("await ")) return false;
...
function isLoginHelperLine(line?: any) {
  const s = line.trim();
  if (!s.startsWith("await ")) return false;
...
function isLoginButtonLine(line?: any) {
  return /getByRole\([^)]*button[^)]*['"]登\s*录['"]|['"]登\s*录['"]\s*\}\s*\)\.click/.test(line);
}

function findLoginBlockEndIndex(awaitIndices?: any, lines?: any) {
 ...
 */
function stripLoginFlowFromRecording(code?: any) {
  ...
  let removed = 0;
  const kept = lines.filter((line: string, i: number) => {
    if (removeSet.has(i)) {
      removed++;
      return false;
    }
 ...
 */
function ensureStorageStateUse(code?: any, storageRel?: any) {
 ...
 * @param {string} code
 */
function ensureInitialNavigation(code?: any) {
 ...
 */
function postprocessRecordedScript(code?: any, opts: any = {}) {
...
export {
  stripLoginFlowFromRecording, ensureStorageStateUse, ensureInitialNavigation, postprocessRecordedScript,
};
