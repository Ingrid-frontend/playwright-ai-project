...
const { readLoginStateMeta, formatLoginAccountLabel } = require("./storage-state-meta.cjs");
const { extractFromCode } = require("./extract-login-account.cjs");
...
function specMetaPathForRel(specRel) {
...
function readSpecMetaFile(repoRoot, specRel) {
...
function writeSpecMetaFile(repoRoot, specRel, meta) {
  const metaRel = specMetaPathForRel(specRel);
...
function deleteSpecMetaFile(repoRoot, specRel) {
  const metaRel = specMetaPathForRel(specRel);
  if (!metaRel) return false;
  const abs = path.join(repoRoot, metaRel);
...
function parseSpecMetaBlockFromCode(code) {
...
function normalizeSpecMeta(meta, { hasMetaFile = true } = {}) {
...
function buildSpecMetaFromSession(repoRoot, opts) {
...
function appendSpecMetaHeaderToCode(code, meta) {
  const payload = {
...
function mapProcessedToOriginalRel(processedRel) {
...
function resolveOptimizedSpecMeta(repoRoot, optimizedRel) {
  ...
  const { parseEnvFromSpecRel, getLegacyEnvDefault } = require("./test-env-path.cjs");
...
function enrichOptimizedSpecEntry(repoRoot, rel) {
...
function copyRawMetaToOptimized(repoRoot, rawRel, optimizedRel, patch = {}) {
...
function summarizeProfileCounts(entries) {
  const counts = {};
...
module.exports = {
  ...
  writeSpecMetaFile,
  deleteSpecMetaFile,
  parseSpecMetaBlockFromCode,
  buildSpecMeta,
  buildSpecMetaFromSession,
  appendSpecMetaHeaderToCode,
  mapProcessedToOriginalRel,
  resolveOptimizedSpecMeta,
  enrichOptimizedSpecEntry,
  copyRawMetaToOptimized,
  writeRawSpecMetaFromSession,
  groupEntriesByAccountProfile,
  summarizeProfileCounts,
};
