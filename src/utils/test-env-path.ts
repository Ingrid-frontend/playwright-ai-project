...
let cachedLayout: typeof DEFAULT_LAYOUT | null = null;
let cachedKnownEnvs: string[] | null = null;

function resolveRepoRoot(startDir?: any) {
...
function loadTestPathLayout(repoRoot?: any) {
...
function listKnownEnvs(repoRoot?: any) {
...
function parseOptimizedRel(relPath?: any, repoRoot?: any) {
...
function parseRawOriginalRel(relPath?: any, repoRoot?: any) {
  const norm = normalizeRel(relPath, repoRoot);
...
function parseEnvFromSpecRel(relPath?: any, repoRoot?: any) {
  const parsed = parseOptimizedRel(relPath, repoRoot);
  return parsed ? parsed.env : null;
}

function buildRawOriginalRel({ playwrightEnv, dateCategory, fileName, repoRoot }: any) {
...
function buildOptimizedRel({ playwrightEnv, dateCategory, stem, repoRoot }: any) {
  const layout = loadTestPathLayout(repoRoot);
  const env = String(playwrightEnv || getLegacyEnvDefault(repoRoot)).trim();
  if (!isKnownEnv(env, repoRoot)) {
...
function buildScreenshotDir({ playwrightEnv, dateCategory, fileName, repoRoot }: any) {
  const layout = loadTestPathLayout(repoRoot);
  const env = String(playwrightEnv || getLegacyEnvDefault(repoRoot)).trim();
  const cat = String(dateCategory || "").trim();
...
function specMatchesEnv(relPath?: any, targetEnv?: any, repoRoot?: any) {
...
function assertSpecEnvMatch(relPath?: any, runtimeEnv?: any, repoRoot?: any) {
...
function optimizedImportDepthFromRel(relPath?: any) {
...
function optimizedImportPathsForDepth(depth?: any) {
...
function rewriteOptimizedSpecImports(code?: any, optimizedRel?: any, repoRoot?: any) {
...
function parseEnvAndDateCategoryFromRawOrProcessed(absOrRel?: any, repoRoot?: any) {
...
export {
  loadTestPathLayout, listKnownEnvs, isKnownEnv, getLegacyEnvDefault, isEnvSegmentEnabled, parseOptimizedRel, parseRawOriginalRel, parseEnvFromSpecRel, buildRawOriginalRel, buildOptimizedRel, buildScreenshotDir, specMatchesEnv, shouldEnforceSpecEnv, assertSpecEnvMatch, optimizedImportDepthFromRel, optimizedImportPathsForDepth, rewriteOptimizedSpecImports, parseEnvAndDateCategoryFromRawOrProcessed, resolveRepoRoot,
};
