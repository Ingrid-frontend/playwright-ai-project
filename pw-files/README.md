...
侧栏 **迭代** 手风琴（亦可从「保存命名」内「管理迭代」进入）以下拉列表展示 `config/date-categories.json` 中的迭代目录（选项仅显示 **6 位 `YYMMDD`**，如 `260116`）；新增须输入合法 `YYMMDD`（校验位数与年月日）；分类规则按完整日历日比较；**仅支持新增**并保存，不可在界面删除。
